 ...
 * @private
 */
Ext.XTemplateParser = function(config) {
    /**
     ...
     * @method
     */
    this.doTpl = function () {}, //Ext.emptyFn,
    ...
    this.topRe =     /(?:(\{\%)|(\{\[)|\{([^{}]+)\})|(?:<tpl([^>]*)\>)|(?:<\/tpl>)/g,
    this.actionsRe = /\s*(elif|elseif|if|for|foreach|exec|switch|case|eval|between)\s*\=\s*(?:(?:"([^"]*)")|(?:'([^']*)'))\s*/g,
    this.propRe =    /prop=(?:(?:"([^"]*)")|(?:'([^']*)'))/,
    this.defaultRe = /^\s*default\s*$/,
    this.elseRe =    /^\s*else\s*$/
};
